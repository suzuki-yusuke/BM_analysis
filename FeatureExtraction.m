...
%             endframe = parameters.fps*150;
            endframe = parameters.fps*parameters.cutoff_probe;
        else % training の場合
        ...
        if strcmp(phase, 'probe_test') % probe test の場合
%             endframe = parameters.fps*150;
            endframe = parameters.fps*parameters.cutoff_prob;
        else % training の場合
    ...
    if options.order{'Network','Analysis'}
        NetworkFeatures = feature_ntw_s(parameters,NetworkFeatures,n,XY);
        disp('Network analysis (static node generation)....DONE.');
        NetworkFeatures = feature_ntw_d(parameters,NetworkFeatures,n,XY);
    ...
    if options.order{'Strategy','Analysis'}&&ismember(phase,{'training','retraining','reversal_training'})
        StrategyFeatures = feature_str(parameters,options,LOG,StrategyFeatures,n,OnlineTracking,XY);
        disp('Strategy analysis..................DONE.');
    else
        disp('Strategy analysis..................SKIP.');
    end
    ...
    if options.order{'Conventional','Analysis'}
        ConventionalFeatures =...
            feature_cnv(parameters, options, ConventionalFeatures, n, phase, XY);
