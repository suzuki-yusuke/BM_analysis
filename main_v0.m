...
options.view_network_graph = true;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if all(options.order.Load)
    ...
    StrategyFeatures{:,{'Random','Serial','Spatial','Perseveration'}} = zeros(N,4);
    ...
    features = {'Sequence', 'travel_distance_prb', 'init_distrib'};
    ExtraFeatures = cell2table(cell(N,length(features))); % Extra
    ExtraFeatures.Properties.VariableNames = features;
...
if options.order{'Conventional','Analysis'} % output data of conventional indices
    summary_cnv_v0(parameters,options,tbl);
else
...
if options.order{'Strategy','Analysis'}&&~ismember('probe_test',unique(tbl.Phase))
    summary_str_v0(parameters,options,tbl)
else
...
if options.order{'Network','Analysis'}
    summary_ntw_v0(parameters,options,tbl)
else
...
if options.order{'Extra','Analysis'}
    summary_ext_v0(parameters,options,tbl)
else
    fprintf('\nAnalyze network indices......SKIP\n')
end
