function [Report,Level,M,N,F,p,MS,df,ES] = anova_1wm(Data)
% 
% anova_1wm:  対応のある1要因分散分析（データは集計表形式）
% 
% [Report,Level,M,N,F,p,MS,df,ES] = anova_1wm(Data)
%    Data    対象となるデータ（集計表形式）
%    Report  分散分析表
%    Level   水準番号の一覧
%    M       各水準の平均値
...
%
% 入力データ（Data）は、サンプル数×水準数の行列です。
% すなわち、同一行内の数値は対応したデータです。
% 4名の参加者による3水準のデータの例：
%       Data = [
%          12.3   18.9   15.5
%          15.1   21.2   16.1
...
%       ];
% 
% see also:   anova_1w   anova_1b   tukey_1wm
% 
...
if ~(size(Data,1)>1 & size(Data,2)>1)
    disp('  [anova_1wm]:  Data must be a matrix.');
    return;
end
