function [Report,Result,Md,t,p,df] = bonf_ttest_2w(Data,Alpha,varargin)
% 
% bonf_ttest_2: ボンフェローニ法による多重比較、対比較はttest_2w
% 
% [Report,Result,Md,t,p,df] = bonf_ttest_2w(Data,Alpha,Method)
%    Data    対象となるデータ（2列の行列）
%    Alpha   有意水準
%    Method  方法を指定します。
%             0 -> ボンフェローニの方法
...
%    df      各対比較の自由度
% 
% 対比較に関数ttest_2w（対応のない2標本のt検定、分散が等質で
% ない場合（ウェルチの検定））を用いて、ボンフェローニ法による
% 多重比較を行います。
% 
...
% なっています。
% 
% この関数は、関数 bonf, list2cell, ttest_2w を利用します。
% 
% see also:   bonf_ttest_2   bonf_ttest_2p   bonf   ttest_2w
% 
...
if 1-isscalar(Alpha)
    disp('  [bonf_ttest_2w]:  Alpha must be a scalar.');
    return;
end
if ((Alpha<0) | (Alpha>1))
    disp('  [bonf_ttest_2w]:  Alpha must be 0~1.');
    return;
end
Method = 0;
if ~isempty(varargin)
...
if sum(Method==[0 1])==0
    disp('  [bonf_ttest_2w]:  Method must be 0 or 1.');
    return;
end    
...
NF = size(Data,2)-1;
if NF ~= 1
    disp('  [bonf_ttest_2w]:  Data must be comprised of 2 columns.');
    return;
end
...
df = zeros(NL);
for I = 1:length(F)
    [Report,t(F(I)),p(F(I)),df(F(I)),Ntemp] = ttest_2w(DataCell{F1(I)},DataCell{F2(I)});
end
